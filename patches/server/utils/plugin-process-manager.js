...
export function startPluginServer(name, pluginDir, serverEntry) {
  ...
  const startPromise = new Promise((resolve, reject) => {
    ...
    // Forward LLM endpoint config and runtime library paths so that processes
    // spawned inside the plugin (e.g. the terminal plugin's PTY shell) can
    // reach the locally-configured model and find native libraries.
    // ANTHROPIC_BASE_URL / ANTHROPIC_API_KEY are required for `claude` to work
    // when launched from within the Terminal plugin's shell.
    const extraEnv = {};
    for (const key of [
      ...
      if (process.env[key] !== undefined) extraEnv[key] = process.env[key];
    }
