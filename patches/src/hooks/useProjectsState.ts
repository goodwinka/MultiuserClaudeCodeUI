...
export function useProjectsState({
  ...
  useEffect(() => {
    ...
    // Multi-user gateway patch: the per-user claudecodeui process may be
    // killed by the 30-minute idle timer and restarted on the next request.
    // WebSocketContext emits a synthetic 'websocket-reconnected' message
    // after the client reconnects; refresh projects so the sidebar catches
    // up on anything that changed while disconnected.
  ...
  // Multi-user gateway patch: belt-and-suspenders auto-refresh.
  // chokidar inotify events are not always reliable across Docker bind mounts
  // and overlay filesystems, so WebSocket 'projects_updated' messages can be
  // missed. Poll every 15s as a fallback, and refresh whenever the tab/window
  // regains focus — both are silent refreshes that keep the sidebar fresh
  // without a visible loading state or manual button click.
  useEffect(() => {
